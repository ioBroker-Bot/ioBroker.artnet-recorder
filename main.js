...
class ArtnetRecorder extends utils.Adapter {
        ...
        try {
            ...
            if (fs.existsSync(this.workingDir)) {
                ...
                if (locWritable == undefined) {
                    ...
                    this.log.info(
                        `Art-Net Recorder working directory: ${this.workingDir} exists but is not writable. Recording not possible.`,
                    );
    ...
    onServerListening() {
        try {
            ...
            this.log.info(
                `Art-Net Recorder server ready on <${addr.address}> port <${addr.port}> proto <${addr.family}>`,
            );
     ...
     *
     * @param msg      the message content received by the server socket
     * @param info     the info for e.g. address of sending host
     */
    async onServerMessage(msg, info) {
        try {
            if (info.address === this.config.bind) {
                return;
            } // packet from own address
            if (msg.length > 530) {
            ...
            if (msg.slice(0, 8).compare(this.artNetHeader) != 0) {
                this.log.debug(
                    `Art-Net Recorder received packet with unknown header. Received header: '${msg.slice(0, 8).toString('hex').toUpperCase()}'`,
                );
                this.log.debug(
                    `-> ${msg.length} bytes from ${info.address}:${info.port} : '${this.logHexData(msg_hex)}'`,
                );
                return;
            }
            if (msg.slice(10, 12).compare(this.artNetVersion) != 0) {
                this.log.debug(
                    `Art-Net Recorder received packet with unknown version. Received version: '${msg.slice(10, 12).toString('hex').toUpperCase()}'`,
                );
                this.log.debug(
                    `-> ${msg.length} bytes from ${info.address}:${info.port} : '${this.logHexData(msg_hex)}'`,
                );
            ...
            if (msg.slice(8, 10).compare(this.artNetOpcodecArtDMX) != 0) {
                this.log.debug(
                    `Art-Net Recorder received packet with unknown opcode. Received opcode: '${msg.slice(8, 10).toString('hex').toUpperCase()}'`,
                );
                this.log.debug(
                    `-> ${msg.length} bytes from ${info.address}:${info.port} : '${this.logHexData(msg_hex)}'`,
                );
            ...
            if (msgNet == this.config.net && msgSubNet == this.config.subNet && msgUniverse == this.config.universe) {
                this.log.silly(
                    `Art-Net Recorder received packet with ${msg.length} bytes from ${info.address}:${info.port} -> sequence: ${msgSequence}, physical: ${msgPhysical}, universe: '${msgNet}:${msgSubNet}:${msgUniverse} (${msgNet * 256 + msgSubNet * 16 + msgUniverse})', length: ${msgLength} DMX values`,
                );
            } else {
                this.log.silly(
                    `Art-Net Recorder received packet with ${msg.length} bytes from ${info.address}:${info.port} -> sequence: ${msgSequence}, physical: ${msgPhysical}, universe: '${msgNet}:${msgSubNet}:${msgUniverse} (${msgNet * 256 + msgSubNet * 16 + msgUniverse})', length: ${msgLength} DMX values. Universe or Net mismatch. Ignoring`,
                );
                return;
            }
            const dmxVals = msg.slice(18, msg.length);
            if (!this.recordRunning) {
                // no recording in progress. Only save the buffer and return
    ...
    onSendTimer() {
        ...
        try {
            if (!this.playbackRunning) {
                return;
            } // only action is while playback is running
            const locNow = Date.now() - this.plyStartTime; // offset !
            if (this.nextPacketTime < locNow) {
                // send packet and get next
                ...
                if (tmpStr) {
                    ...
                    if (this.plyLoop) {
                        ...
                        if (tmpStr) {
                            ...
                            this.log.error(
                                `Art-Net Recorder error: File '${this.plyFile}' is now empty or corrupted. No playback possible`,
                            );
                            ...
                            switch (state.val) {
                                ...
                                case 2: // Playback
                                    ...
                                    if (fs.existsSync(this.plyFile)) {
                                        this.liner = new lineByLine(this.plyFile, {
                                            readChunk: 15000,
                                            newLineCharacter: '\n',
                                        });
                                        ...
                                        if (tmpStr) {
                                            ...
                                            this.log.error(
                                                `Art-Net Recorder error: File '${this.plyFile}' is empty or corrupted. No playback possible`,
                                            );
                                            this.playbackRunning = false;
                                            if (this.liner) {
                                                this.liner.close();
                                        ...
                                        this.log.error(
                                            `Art-Net Recorder error: File '${this.plyFile}' does not exist. No playback possible`,
                                        );
                                        this.playbackRunning = false;
                                        this.setState('control.mode', 0, true);
                                    }
    ...
    sendPacket() {
        try {
            ...
            for (const element of this.nextPacketToSend) {
                if (this.mergeMode) {
                    // if 0 (LTP) the provided values from the playback must be set
                    this.artNetBuffer[element.channel - 1] = Number(element.value);
                } else {
                    // HTP only set value if it is bigger than the bufferd value
     ...
     *
     * @returns the buffer with the artnet packet
     */
    getArtNetPacket() {
        ...
        try {
            ...
            */
            ...
            locBuffer = Buffer.concat([
                locBuffer,
                new Uint8Array([Number(universeHiByte), this.config.net, Number(lengthHiByte), Number(lengthLoByte)]),
                this.artNetBuffer,
            ]);
        } catch (err) {
     ...
     *
     * @param locStr the string to format
     */
    logHexData(locStr) {
        let retStr = '';
     ...
     * @param module module in which the error occurs
     */
     ...
     *
     * @param callback function gives by js-controller at the end of the process lifetime
     */
    onUnload(callback) {
        try {
            // Reset the connection indicator
